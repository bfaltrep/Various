...
{
  int index = get_global_id (0); //id thread
  calc_t p = pos[index];
  calc_t sp = speed[index];

  int axis = index/offset;

  sp *= (p < min[axis]) || (p > max[axis]) ? -1.0 : 1.0;
...
{
  coord_t p1, p2;
  p1 = load3coord(&pos[index],offset);

  for(int i = index+1 ; i < natoms ; i ++)
  {
    p2 = load3coord(&pos[i],offset);
	      ...
	      unsigned natoms, unsigned offset)
{
  int index = get_global_id (0); 
  if(gy > 0){
    speed[index+offset] = speed[index+offset] - 0.005;
  }
}
