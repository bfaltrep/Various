...
void run_workers(void *(thread_fun)(void *param), int nb)
{
  int i;
  ...
  for (i=0; i<nbWorkers; i++)
    {
      ...
      pthread_create(&workers[i].tid,&workers[i].attr,thread_fun,&workers[i]);

    }
}
 
void go(void *(*fun)(void *p),void *p, int nb)
{
  run_workers(worker_main, nb);
