...
Dupliquer le répertoire source initial et parallélisez-la à l'aide de
tâches en faisant en sorte de ne créer des tâches parallèles que
jusqu'au niveau \texttt{grain}.

Pourquoi faut-il ici allouer dynamiquement la mémoire du chemin
emprunté ?  Comment limiter allocations et copies à tous niveaux ?
Quand désallouer ?.

On pourra mesurer l'influence des clauses \texttt{mergeable} (limitant le
coût de génération d'une tâche) et \texttt{final(condition)} pour créer
une tâche parallèle qui sera elle exécutée séquentientiellement.
\end{document}
