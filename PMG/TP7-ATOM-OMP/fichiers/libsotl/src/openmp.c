...
#pragma omp parallel for schedule (static,1)
  for (unsigned n = 0; n < set->natoms; n++) {
    ...
    // Atom color depends on z coordinate
    {
      ...
      vbo_color[n*3 + 0] = (omp_get_thread_num()%7)/6.0;

      vbo_color[n*3 + 1] = (omp_get_thread_num()%7)/4.0;
      vbo_color[n*3 + 2] = (omp_get_thread_num()%7)/2.0;
      atom_state[n]--;
    }
  }
}
...
#pragma omp parallel for schedule (static,1)
  for (unsigned n = 0; n < set->natoms; n++) {
    if(set->pos.x[n] >= domain->max_ext[0] || set->pos.x[n] <= domain->min_ext[0]){
      set->speed.dx[n] *= -1;
    }
    if(set->pos.y[n] >= domain->max_ext[1] || set->pos.y[n] <= domain->min_ext[1]){
      set->speed.dy[n] *= -1;
      if(gravity_enabled){
        (set->pos.y[n] > domain->max_ext[1]) ? (set->pos.y[n] = domain->max_ext[1]-0.1) : (set->pos.y[n] = domain->min_ext[1]+0.1) ;
      } 
    }
    if(set->pos.z[n] >= domain->max_ext[2] || set->pos.z[n] <= domain->min_ext[2]){
      set->speed.dz[n] *= -1;
    }
  }
}
