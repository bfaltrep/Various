  ...
  if (myid == 0)
    {
      ...
      for(i=1;i<numprocs;i++){
        ...
        MPI_Send(a+i*tranche,N*tranche,MPI_CHAR,i,1,MPI_COMM_WORLD);
      }
      ...
      for(i=1;i<numprocs;i++){
        MPI_Recv(c+i*tranche,N*tranche,MPI_CHAR,i,2,MPI_COMM_WORLD, &status);
      }
  ...
  else
    {
      ...
      MPI_Recv(a,N*tranche,MPI_CHAR,0,1,MPI_COMM_WORLD, &status);

      calculer(a,b,c,tranche);

     // envoyer c
      MPI_Send(c,N*tranche,MPI_CHAR,0,2,MPI_COMM_WORLD);
